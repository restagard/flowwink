...
const PURCHASING_SKILLS: SkillSeed[] = [
  ...
  },
  {
    ...
    description: 'Change an existing purchase order and record the revision in one step: quantities, prices, added or removed lines, delivery date, notes. Use when: the vendor changes a price, you need more or fewer units, a line is dropped, delivery moves — on a draft, sent, confirmed or partially received order. Every amendment stores a before/after snapshot with its reason, and an amount that grows past what was approved opens a new approval request. NOT for: status changes (update_purchase_order), receiving goods (receive_purchase_order), or a received/cancelled order (create a new one).',
    category: 'commerce',
    handler: 'rpc:amend_purchase_order',
    scope: 'internal',
    trust_level: 'notify',
    tool_definition: {"type": "function", "function": {"name": "amend_purchase_order", "description": "Amend a purchase order (lines, delivery date, notes) and record the revision with its reason.", "parameters": {"type": "object", "required": ["p_purchase_order_id", "p_reason"], "properties": {"p_purchase_order_id": {"type": "string", "description": "UUID of the purchase order"}, "p_reason": {"type": "string", "description": "Why the order changes — stored on the revision"}, "p_lines": {"type": "array", "description": "Only the lines that change. With line_id: update that line (send remove:true to drop it). Without line_id: a new line.", "items": {"type": "object", "properties": {"line_id": {"type": "string"}, "remove": {"type": "boolean"}, "product_id": {"type": "string"}, "description": {"type": "string"}, "quantity": {"type": "number"}, "unit_price_cents": {"type": "number"}, "tax_rate": {"type": "number", "description": "Percent. A new line inherits the order's rate when omitted."}}}}, "p_expected_delivery": {"type": "string", "description": "New expected delivery date YYYY-MM-DD"}, "p_notes": {"type": "string"}}}}} as SkillSeed['tool_definition'],
    instructions: 'Read the order first with update_purchase_order action:"get" — the line ids are purchase_order_lines[].id. Send ONLY the lines that change; untouched lines stay as they are. Floors: a quantity cannot go below received_quantity, and a line with anything received cannot be removed. A new line needs unit_price_cents (a price is never guessed). An amendment that changes nothing is refused and records no revision. The answer carries revision_number, prev/new totals, approval_request_id when the growth needs re-approval, and the whole order after the change. When the order was already sent, tell the vendor — the answer says so.',
  },
  {
    name: 'list_po_revisions',
    description: 'List the amendment history of one purchase order: revision number, reason, before/after totals and snapshots. Use when: "what changed on this PO?", auditing why a total differs from the first version. NOT for: making a change (amend_purchase_order).',
    category: 'commerce',
    handler: 'rpc:list_po_revisions',
    scope: 'internal',
    trust_level: 'notify',
    tool_definition: {"type": "function", "function": {"name": "list_po_revisions", "description": "List the recorded revisions of a purchase order, newest first.", "parameters": {"type": "object", "required": ["p_purchase_order_id"], "properties": {"p_purchase_order_id": {"type": "string"}}}}} as SkillSeed['tool_definition'],
    instructions: 'Each revision carries snapshot.prev and snapshot.next — the full order (header + lines) before and after.',
  },
  {
    name: 'vendor_scorecard',
    description: 'Vendor performance: on-time delivery %, order count, invoice variance rate and the manual rating, per vendor. Use when: choosing between suppliers, reviewing a vendor before renewing, "which vendors deliver late?". NOT for: vendor master data (manage_vendor) or prices (manage_vendor_price).',
    category: 'commerce',
    handler: 'rpc:vendor_scorecard',
    scope: 'internal',
    trust_level: 'notify',
    tool_definition: {"type": "function", "function": {"name": "vendor_scorecard", "description": "Read the vendor scorecard for one vendor or all.", "parameters": {"type": "object", "required": [], "properties": {"p_vendor_id": {"type": "string", "description": "Omit for all vendors"}, "p_limit": {"type": "number"}}}}} as SkillSeed['tool_definition'],
    instructions: 'on_time_pct is computed from the first goods receipt against the expected delivery date, and is NULL until an order with an expected date has been received — NULL means "no evidence yet", not a bad score. Set the human judgement with rate_vendor.',
  },
  {
    name: 'rate_vendor',
    description: 'Set or clear the manual rating (0–5) and rating notes on a vendor. Use when: a buyer records a judgement the numbers do not show (responsiveness, quality of support). NOT for: the computed delivery/invoice metrics (vendor_scorecard reads those; they cannot be written).',
    category: 'commerce',
    handler: 'rpc:rate_vendor',
    scope: 'internal',
    trust_level: 'notify',
    tool_definition: {"type": "function", "function": {"name": "rate_vendor", "description": "Set a vendor's manual rating 0–5 (null clears it) with optional notes.", "parameters": {"type": "object", "required": ["p_vendor_id"], "properties": {"p_vendor_id": {"type": "string"}, "p_rating": {"type": "number", "description": "0–5; omit or null to clear"}, "p_notes": {"type": "string"}}}}} as SkillSeed['tool_definition'],
    instructions: 'The rating is a human judgement stored beside the computed metrics — it never changes them.',
  },
  {
    name: 'open_vendor_dispute',
    description: 'Open a dispute on a vendor invoice: wrong price, damaged or missing goods, a duplicate bill. Holds the payment — pay_vendor_invoice refuses a bill under open dispute. Use when: a supplier bill is wrong and should not be paid as it stands. NOT for: customer invoice disputes, or a bill that merely fails the three-way match (match_invoice_to_receipt explains that).',
    category: 'commerce',
    handler: 'rpc:open_vendor_dispute',
    scope: 'internal',
    trust_level: 'notify',
    tool_definition: {"type": "function", "function": {"name": "open_vendor_dispute", "description": "Open a dispute on a vendor invoice and hold its payment.", "parameters": {"type": "object", "required": ["p_vendor_invoice_id", "p_reason"], "properties": {"p_vendor_invoice_id": {"type": "string"}, "p_reason": {"type": "string"}, "p_disputed_amount_cents": {"type": "number", "description": "Gross amount in dispute; omit when the whole bill is disputed"}}}}} as SkillSeed['tool_definition'],
    instructions: 'Idempotent: a bill has at most one open dispute, and a second call returns it (already_open:true). Close it with resolve_vendor_dispute.',
  },
  {
    name: 'resolve_vendor_dispute',
    description: 'Close a vendor invoice dispute, and when the vendor credits part of the bill, issue and book that credit memo in the same step. Use when: the vendor agreed to a credit, the bill turned out right after all (resolve without credit), or the dispute was opened by mistake (p_outcome "cancelled"). NOT for: opening one (open_vendor_dispute) or a credit that has no dispute (issue_vendor_credit_memo).',
    category: 'commerce',
    handler: 'rpc:resolve_vendor_dispute',
    scope: 'internal',
    trust_level: 'notify',
    tool_definition: {"type": "function", "function": {"name": "resolve_vendor_dispute", "description": "Resolve or cancel a vendor dispute; optionally issue and book the vendor's credit.", "parameters": {"type": "object", "required": ["p_dispute_id", "p_resolution"], "properties": {"p_dispute_id": {"type": "string"}, "p_resolution": {"type": "string", "description": "How it ended"}, "p_outcome": {"type": "string", "enum": ["resolved", "cancelled"]}, "p_credit_amount_cents": {"type": "number", "description": "Gross credit the vendor grants (VAT included)"}, "p_credit_number": {"type": "string", "description": "The vendor's own credit memo number, when known"}}}}} as SkillSeed['tool_definition'],
    instructions: 'With p_credit_amount_cents the credit memo is issued against the disputed bill and booked at once: payables debited, input VAT reversed in the bill\'s proportion, the rest against cost (or purchase price variance when the bill belongs to an order). After that pay_vendor_invoice pays the total minus the credit. Everything happens in one transaction — if the credit cannot be booked the dispute stays open.',
  },
  {
    name: 'issue_vendor_credit_memo',
    description: 'Register a credit memo received from a vendor and book it. Use when: a supplier sends a credit note — returned goods, a price correction, a goodwill credit — against a specific bill (p_vendor_invoice_id) or against the vendor in general (p_vendor_id). NOT for: crediting a CUSTOMER invoice (credit_invoice), or a credit that settles a dispute (resolve_vendor_dispute does both).',
    category: 'commerce',
    handler: 'rpc:issue_vendor_credit_memo',
    scope: 'internal',
    trust_level: 'notify',
    tool_definition: {"type": "function", "function": {"name": "issue_vendor_credit_memo", "description": "Register a vendor credit memo; booked immediately unless p_apply is false.", "parameters": {"type": "object", "required": ["p_amount_cents", "p_reason"], "properties": {"p_amount_cents": {"type": "number", "description": "Gross, VAT included"}, "p_reason": {"type": "string"}, "p_vendor_invoice_id": {"type": "string", "description": "The unpaid bill it reduces"}, "p_vendor_id": {"type": "string", "description": "Use instead of p_vendor_invoice_id for a credit against the vendor"}, "p_dispute_id": {"type": "string"}, "p_credit_number": {"type": "string", "description": "The vendor's credit memo number — the same number twice is the same memo"}, "p_credit_date": {"type": "string"}, "p_apply": {"type": "boolean", "description": "Default true: book it now"}}}}} as SkillSeed['tool_definition'],
    instructions: 'The amount is gross. Against a bill: the credits on one bill can never exceed its total, and a PAID bill cannot be credited — issue the credit against the vendor instead (it then stands as a claim in accounts payable). Idempotent on p_credit_number. The three-way match counts applied credits, so crediting an over-invoiced bill down to what was ordered is what makes it payable.',
  },
  {
    name: 'apply_vendor_credit_memo',
    description: 'Book a vendor credit memo that was registered without being applied (status issued). Use when: a memo was issued with p_apply:false or created in the admin panel and should now reduce the debt. NOT for: registering a new memo (issue_vendor_credit_memo books it directly).',
    category: 'commerce',
    handler: 'rpc:apply_vendor_credit_memo',
    scope: 'internal',
    trust_level: 'notify',
    tool_definition: {"type": "function", "function": {"name": "apply_vendor_credit_memo", "description": "Book an issued vendor credit memo.", "parameters": {"type": "object", "required": ["p_credit_memo_id"], "properties": {"p_credit_memo_id": {"type": "string"}, "p_entry_date": {"type": "string", "description": "Booking date YYYY-MM-DD (default: the memo's date)"}}}}} as SkillSeed['tool_definition'],
    instructions: 'Idempotent: an already applied memo returns its journal entry. An applied memo is final — the table refuses edits; correct it with a new memo.',
  },
  {
    name: 'manage_vendor',
