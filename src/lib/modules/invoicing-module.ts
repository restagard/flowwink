...
Locale-specific: ${getActivePack().ai_instructions.invoicing}`,
  },
  {
      ...
      function: {
        ...
        parameters: {
          ...
          properties: {
            ...
            tax_rate: { type: 'number', description: 'VAT rate for the invoice, as a fraction (0.25, 0.06) or a percent (25, 6). Omit for the instance default.' },
    ...
    instructions: 'Aggregate billable hours from time_entries for the given project and period. Each entry becomes a line item with hours × project hourly rate. Group options: "entry" (one line per entry), "user" (sum per user), "week" (sum per week). Auto-set due_date to issue_date + due_days. The customer is the PROJECT\'s partner (manage_project partner_id): the draft carries their name, e-mail and partner_id — a project without a partner gives an invoice with no recipient, so set it first. VAT is tax_rate, else the instance default; never assumed.',
  },
  {
      ...
      function: {
        ...
        parameters: {
          ...
          properties: {
            ...
            tax_rate: { type: 'number', description: 'VAT rate as a fraction (0.25, 0.06) or a percent (25, 6). Omit for the instance default.' },
          },
  ...
  },
  {
    ...
    category: 'commerce',
    handler: 'internal:send_dunning_reminders',
    scope: 'external',
    tool_definition: {
      type: 'function',
      function: {
