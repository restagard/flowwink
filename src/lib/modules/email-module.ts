...
- When diagnosing a failed send — error_message and metadata.provider_response often hold the root cause.`,
  ...
  {
    name: 'reply_to_email',
    description: 'Send a reply on an email thread on the platform email rail (threaded via Composio/Gmail when connected) and log it on the thread. This is the one send in the FlowPilot-first mail rail, so its trust level is the dial: approve (default) stages the send in Approvals and FlowBox; auto sends at once. Use when: draft_email_reply on a reply_mode=ai_first mailbox has an answer to send; a person approves a staged reply. NOT for: composing the answer (draft_email_reply and the responder do that), newsletters, outbound prospecting (send_email_to_lead), ticket replies.',
    category: 'communication',
    handler: 'internal:reply_to_email',
    scope: 'both',
    trust_level: 'approve',
    instructions: 'Arguments: to, subject, body_text, thread_id, in_reply_to, references, replied_to (inbound message id — idempotency: a second send for the same message is skipped), draft_id (marked used after the send), related_entity_type/id. Delivered through email-send with source flowpilot-reply; the log row lands on the thread. On trust=approve the call returns 202 pending_approval; Approve in Approvals, in the Skill Hub, or Send on the prefilled draft in FlowBox re-runs it with _approved.',
    tool_definition: {
      type: 'function',
      function: {
        name: 'reply_to_email',
        description: 'Send a reply on an email thread (platform email rail, threaded). Trust-gated: approve by default.',
        parameters: {
          type: 'object',
          properties: {
            to: { type: 'string', description: 'Recipient address (REQUIRED).' },
            body_text: { type: 'string', description: 'Plain-text reply body (REQUIRED). Paragraphs separated by blank lines.' },
            subject: { type: 'string', description: 'Subject, normally "Re: …".' },
            thread_id: { type: 'string', description: 'Thread key the reply belongs to.' },
            in_reply_to: { type: 'string', description: 'RFC822 Message-ID of the mail being answered.' },
            references: { type: 'string', description: 'References header chain.' },
            replied_to: { type: 'string', description: 'Inbound message id being answered — idempotency key.' },
            draft_id: { type: 'string', description: 'The draft row this send fulfils; marked used afterwards.' },
